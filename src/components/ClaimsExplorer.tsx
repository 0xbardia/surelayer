...
export function ClaimsExplorer() {
  ...
  return (
    <section aria-live="polite">
      <div className="toolbar"><span className="toolbar-note">Final chain state only · page {Math.floor(offset / limit) + 1}</span><Link href="/create" className="button secondary">Issue a warranty</Link></div>
      ...
      <div className="claim-grid mt-24">
        {claims.map((claim) => <Link className="claim-row" href={`/claims/${claim.id}`} key={claim.id}>
          ...
          <div><h3>{claim.statement}</h3><p>{claim.issuer}</p>{claim.protocolTest ? <p className="demo-label">Protocol test claim · not usage data</p> : null}</div>
          <span className="claim-meta">{formatGen(claim.claimBond)} GEN<br />{formatDate(claim.createdAt)}</span>
