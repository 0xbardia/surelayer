...
export function AccountPanel() {
  ...
  return <div className="split account-ledger-grid"><div className="ledger-balance"><div className="ledger-heading"><div className="eyebrow">Personal ledger</div><span className="record-label">Live / B</span></div><h2>Available credit</h2><p className="big-number">{credit === null ? "—" : `${formatGen(credit)} GEN`}</p><div className="ledger-rule" aria-hidden="true"><span /></div><p className="muted">Only finalized protocol credits are shown. The contract zeroes a credit before attempting the external transfer.</p>{error ? <StateNotice kind="error" title={txState === "success" ? "Ledger refresh delayed" : "Account action unavailable"}>{error}</StateNotice> : null}</div><div className="form-card account-action-card">{address ? <><div className="account-address"><span className="eyebrow">Connected wallet</span><p className="mono small">{address}</p></div><p className="muted small">Withdrawals are signed by your wallet and settle only after GenLayer finality.</p><button className="button" type="button" onClick={() => void withdraw()} disabled={credit === null || credit === 0n || txState === "submitting" || txState === "waiting" || txState === "undetermined"}>{txState === "submitting" ? "Confirm in wallet…" : txState === "waiting" ? "Waiting for finality…" : txState === "undetermined" ? "Read transaction before retrying" : "Withdraw available credit"}</button></> : <><p className="eyebrow">Wallet boundary</p><h2>Connect to inspect your ledger</h2><p className="muted">SureLayer never asks the server to custody your wallet or private key.</p><button className="button" type="button" onClick={() => void connect()}>Connect wallet</button></>}<TransactionStatus state={txState} hash={txHash} /></div></div>;
}
