    ...
    <div className="container section detail-shell">
      <div className="detail-grid">
        <article className="detail-main">
          ...
          <section className="detail-block"><h2>Artifact</h2><dl className="data-table"><div className="data-cell"><dt>Reference</dt><dd>{safeHttp(claim.artifactRef) ? <a className="source-link" href={claim.artifactRef} target="_blank" rel="noopener noreferrer">{claim.artifactRef} ↗</a> : claim.artifactRef || "Not supplied"}</dd></div><div className="data-cell"><dt>Content hash</dt><dd className="mono">{claim.artifactHash || "Not supplied"}</dd></div><div className="data-cell"><dt>Artifact integrity</dt><dd>{claim.artifactIntegrity || "Not evaluated"}</dd></div></dl></section>
          <section className="detail-block"><h2>Evidence sources</h2><SourceGroup label="Issuer sources" sources={claim.issuerSources} hashes={claim.issuerHashes} /><SourceGroup label="Challenger sources" sources={claim.challengerSources} hashes={claim.challengerHashes} /></section>
...
function SourceGroup({ label, sources, hashes }: { label: string; sources: string[]; hashes: string[] }) {
  return <div className="mt-24"><h3 className="small muted">{label}</h3>{sources.length ? <ul className="source-list">{sources.map((source, index) => <li key={`${source}-${index}`}><div>{safeHttp(source) ? <a className="source-link" href={source} target="_blank" rel="noopener noreferrer">{source} ↗</a> : source}</div><span className="mono small">{hashes[index] || "Hash unavailable"}</span></li>)}</ul> : <p className="muted small">None supplied.</p>}</div>;
}
